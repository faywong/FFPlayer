...
public class GLSurfaceView_SDL extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private static class ComponentSizeChooser extends BaseConfigChooser {
        public ComponentSizeChooser(int redSize, int greenSize, int blueSize, int alphaSize,
                int depthSize, int stencilSize, boolean isGles2) {
        ...
        public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display, EGLConfig[] configs) {
            ...
            Log.v("SDL", "Desired GL config: " + "R" + mRedSize + "G" + mGreenSize + "B"
                    + mBlueSize + "A" + mAlphaSize + " depth " + mDepthSize + " stencil "
                    + mStencilSize + " type " + (mIsGles2 ? "GLES2" : "GLES"));
            for (EGLConfig config : configs) {
                ...
                int rendertype = findConfigAttrib(egl, display, config, EGL10.EGL_RENDERABLE_TYPE,
                        0);
                ...
                int caveat = findConfigAttrib(egl, display, config, EGL10.EGL_CONFIG_CAVEAT,
                        EGL10.EGL_NONE);
                int distance = Math.abs(r - mRedSize) + Math.abs(g - mGreenSize)
                        + Math.abs(b - mBlueSize) + Math.abs(a - mAlphaSize);
                int dist2 = distance;
                if ((d > 0) != (mDepthSize > 0))
                    distance += (mDepthSize > 0) ? 5 : 1; // Small penalty if we
                                                          // don't need zbuffer
                                                          // but it is present
                int dist3 = distance;
                ...
                if (caveat == EGL10.EGL_SLOW_CONFIG)
                    distance += 4;
                if (caveat == EGL10.EGL_NON_CONFORMANT_CONFIG) // dunno what
                                                               // that means,
                                                               // probably R and
                                                               // B channels
                                                               // swapped
                ...
                String cfgcur = "R" + r + "G" + g + "B" + b + "A" + a + " depth " + d + " stencil "
                        + s + " type " + rendertype + " (";
                ...
                cfgcur += " pos " + distance + " (" + dist2 + "," + dist3 + "," + dist4 + ","
                        + dist5 + ")";
            ...
            Log.v("SDL", "GLSurfaceView_SDL::EGLConfigChooser::chooseConfig(): selected "
                    + selectidx + ": " + cfglog);
        ...
        private int findConfigAttrib(EGL10 egl, EGLDisplay display, EGLConfig config,
            ...
            Log.w("SDL",
                    "GLSurfaceView_SDL::EGLConfigChooser::findConfigAttrib(): attribute doesn't exist: "
                            + attribute);
            return defaultValue;
        }
    ...
    private static ComponentSizeChooser getEglConfigChooser(int videoDepthBpp,
            boolean withDepthBuffer, boolean stencil, boolean gles2) {
        if (videoDepthBpp == 16)
    ...
    private class EglHelper {
        ...
        public void start() {
            ...
            if (mEglContext == null || mEglContext == EGL10.EGL_NO_CONTEXT)
                Log.e("SDL",
                        "GLSurfaceView_SDL::EglHelper::start(): mEglContext is EGL_NO_CONTEXT, error: "
                                + mEgl.eglGetError());
        ...
        public GL createSurface(SurfaceHolder holder) {
             ...
             */
            /*
             * // This does not have any effect on Galaxy Note int [] attribList
             * = new int[4]; attribList[0] = mEgl.EGL_RENDER_BUFFER;
             * attribList[1] = mEgl.EGL_SINGLE_BUFFER; attribList[2] =
             * mEgl.EGL_NONE; attribList[3] = mEgl.EGL_NONE;
             */
    ...
    class GLThread extends Thread implements SwapBuffersCallback {
        ...
        public boolean SwapBuffers() {
            ...
            while (true) { // Loop until we're re-created GL context and
                ...
                synchronized (this) {
                    /*
                     * Runnable r; while ((r = getEvent()) != null) { r.run(); }
                     */
                    if (mPaused) {
                        mRenderer.onSurfaceDestroyed();
                        mEglHelper.finish();
                        mNeedStart = true;
                        if (Settings.NonBlockingSwapBuffers)
                            return false;
                    }
                    while (needToWait()) {
