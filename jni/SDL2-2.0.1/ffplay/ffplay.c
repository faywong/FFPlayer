...
#define fprintf(X, ...) __android_log_print(ANDROID_LOG_INFO, "FFplay", __VA_ARGS__)
                                  ...
                                  int x, int y, int w, int h, int color, int update)
{
    ...
    rect.w = w;
    rect.h = h;
	// TODO: not to ignore color param
...
static void alloc_picture(VideoState *is)
{
    ...
    video_open(is, 0, vp);
    vp->texture = SDL_CreateTexture(render,
                                SDL_PIXELFORMAT_YV12,
                                SDL_TEXTUREACCESS_STREAMING,
                                vp->width, 
								vp->height);
#if 0
    bufferdiff = vp->texture ? FFMAX(vp->texture->pixels[0], vp->texture->pixels[1]) - FFMIN(vp->texture->pixels[0], vp->texture->pixels[1]) : 0;
    if (!vp->texture || vp->texture->pitches[0] < vp->width || bufferdiff < (int64_t)vp->height * vp->texture->pitches[0]) {
...
#if 0
static void duplicate_right_border_pixels(SDL_Renderer *bmp) {
    int i, width, height;
    Uint8 *p, *maxp;
    for (i = 0; i < 3; i++) {
        width  = bmp->w;
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts, int64_t pos, int serial)
{
    ...
    if (!vp->texture || vp->reallocate || !vp->allocated ||
    ...
    if (vp->texture) {
        ...
        SDL_LockTexture (vp->texture, NULL, (void **)&pixels, &pitches);
        ...
        //duplicate_right_border_pixels(vp->texture);
