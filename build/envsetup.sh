...
function copy_assert()
{
    ...
    echo "Copying app data files from project/AndroidData to project/assets"
    mkdir -p project/assets
    rm -f project/assets/*
    if [ -d "project/AndroidData" ] ; then
            cp project/AndroidData/* project/assets/
...
#    else
#        print_lunch_menu
#        echo -n "Which would you like? [ffplay] "
#        read answer
#    fi
...
# Do not rebuild several huge libraries that do not depend on SDL version
    ...
    if which convert > /dev/null; then
            ...
            convert project/res/drawable/icon.png -resize '732x412!' project/res/drawable-xhdpi/icon.png
    else
...
# check PATH setting & build utils
    check_cmd_exist android || return 1
    check_cmd_exist ndk-build || return 1
    check_cmd_exist ant || return 1
    check_cmd_exist aapt || return 1
...
#check_aapt
    ...
    echo
    android list target | grep "API level: .*[1-9][0-9]"

    echo -n "Which android sdk version/API level do you want to apply?"
    read sdk_version
    android update project -p $T/project -t android-$sdk_version -s
    if [ $? -ne 0 ]; then
...
#            android update project -p project || return 1
...
#                    [ "`readlink $T/project/AndroidAppSettings.cfg`" -ot "$T/project/src/Globals.java" ] && \
...
#            ./AndroidPreBuild.sh || { echo "AndroidPreBuild.sh returned with error" ; return 1 ; }
#            cd $T
#    }
...
cd $T/project && env PATH=$NDKBUILDPATH BUILD_NUM_CPUS=$NCPU nice -n19 ndk-build -j$NCPU && \
	{	grep "CustomBuildScript=y" $T/project/AndroidAppSettings.cfg > /dev/null && \
		...
		|| true ; } && \
	{	grep "CustomBuildScript=y" $T/project/AndroidAppSettings.cfg > /dev/null && \
		grep "MultiABI=y" $T/project/AndroidAppSettings.cfg > /dev/null && \
		...
		|| true ; } && \
	cd .. && copy_assert && cd $T/project && \
	{	if $build_release ; then \
			ant release || return 1 ; \
			jarsigner -verbose -keystore ~/.android/debug.keystore -storepass android -sigalg MD5withRSA -digestalg SHA1 bin/MainActivity-release-unsigned.apk androiddebugkey || return 1 ; \
	...
	{	cd bin && adb install -r MainActivity-debug.apk | grep 'Failure' && \
		adb uninstall `grep AppFullName ../$T/project/AndroidAppSettings.cfg | sed 's/.*=//'` && adb install -r MainActivity-debug.apk ; true ; } && \
	$run_apk && { \
		ActivityName="`grep AppFullName ../$T/project/AndroidAppSettings.cfg | sed 's/.*=//'`/.MainActivity" ; \
