...
import org.ffmpeg.ffplayer.app.*;
import org.ffmpeg.ffplayer.ui.ISortable;
...
public class DirectoryViewFragment extends SherlockListFragment implements ISortable {
    public final static String TAG = "FFPlayer/DirectoryViewFragment";
...
//            AudioServiceController.getInstance().append(mediaLocation);
    ...
    private void openMediaFile(String path) {
        Log.d(TAG, "The target mediafile to be opened is:" + path);
        Intent launchFFPlayer = new Intent(Intent.ACTION_VIEW);
        launchFFPlayer.addCategory(Intent.CATEGORY_DEFAULT);
        launchFFPlayer.setClass(getActivity(), FFPlayer.class);
        launchFFPlayer.setData(Uri.parse(path));
    }
