...
public class SDLInput {
    ...
    abstract static class DifferentTouchInput {
        ...
        private static class SingleTouchInput extends DifferentTouchInput {
            ...
            public void process(final MotionEvent event) {
                ...
                if (action >= 0)
                    FFPlayerView.nativeMotionEvent((int) event.getX(), (int) event.getY(), action,
                            0, (int) (event.getPressure() * 1000.0),
                            (int) (event.getSize() * 1000.0));
            }
        }

        private static class MultiTouchInput extends DifferentTouchInput {
            public static final int TOUCH_EVENTS_MAX = 16; // Max multitouch
                                                           // pointers
            ...
            public void process(final MotionEvent event) {
                ...
                // System.out.println("Got motion event, type " +
                // (int)(event.getAction()) + " X " + (int)event.getX() + " Y "
                // + (int)event.getY());
                if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP
                    ...
                    for (int i = 0; i < TOUCH_EVENTS_MAX; i++) {
                        if (touchEvents[i].down) {
                            ...
                            FFPlayerView.nativeMotionEvent(touchEvents[i].x, touchEvents[i].y,
                                    action, i, touchEvents[i].pressure, touchEvents[i].size);
                        }
                    }
                }
                if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_DOWN) {
                    ...
                    for (int i = 0; i < event.getPointerCount(); i++) {
                        ...
                        FFPlayerView.nativeMotionEvent(touchEvents[id].x, touchEvents[id].y,
                                action, id, touchEvents[id].pressure, touchEvents[id].size);
                    }
                }
                if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_MOVE
                    ...
                    /*
                     * String s = "MOVE: ptrs " + event.getPointerCount(); for(
                     * int i = 0 ; i < event.getPointerCount(); i++ ) { s +=
                     * " p" + event.getPointerId(i) + "=" + (int)event.getX(i) +
                     * ":" + (int)event.getY(i); } System.out.println(s);
                     */
                    ...
                    for (int id = 0; id < TOUCH_EVENTS_MAX; id++) {
                        ...
                        if (ii >= event.getPointerCount()) {
                            ...
                            if (touchEvents[id].down) {
                                ...
                                FFPlayerView.nativeMotionEvent(touchEvents[id].x,
                                        touchEvents[id].y, action, id, touchEvents[id].pressure,
                                        touchEvents[id].size);
                            }
                        } else {
                                                                                                    ...
                                                                                                    // 3.1
                {
                    ...
                    FFPlayerView.nativeMotionEvent(touchEvents[0].x, touchEvents[0].y, action, 0,
                            touchEvents[0].pressure, touchEvents[0].size);
                }
            }
        }
        ...
        private static class GingerbreadTouchInput extends MultiTouchInput {
			...
			public void process(final MotionEvent event) {
                boolean hwMouseEvent = ((event.getSource() & InputDevice.SOURCE_MOUSE) == InputDevice.SOURCE_MOUSE
                        || (event.getSource() & InputDevice.SOURCE_STYLUS) == InputDevice.SOURCE_STYLUS || (event
                        .getMetaState() & KeyEvent.FLAG_TRACKING) != 0); // Hack
                                                                         // to
                                                                         // recognize
                                                                         // Galaxy
                                                                         // Note
                ...
                if (ExternalMouseDetected != hwMouseEvent) {
                    ...
                    FFPlayerView.nativeHardwareMouseDetected(hwMouseEvent ? 1 : 0);
                }
                super.process(event);
            }
        ...
        private static class IcsTouchInput extends GingerbreadTouchInput {
			...
			public void process(final MotionEvent event) {
                // System.out.println("Got motion event, type " +
                // (int)(event.getAction()) + " X " + (int)event.getX() + " Y "
                // + (int)event.getY() + " buttons " + buttonState + " source "
                // + event.getSource());
                ...
                if (buttonStateNew != buttonState) {
                    for (int i = 1; i <= MotionEvent.BUTTON_FORWARD; i *= 2) {
                        if ((buttonStateNew & i) != (buttonState & i))
                            FFPlayerView.nativeMouseButtonsPressed(i,
                                    ((buttonStateNew & i) == 0) ? 0 : 1);
                    }
            ...
            public void processGenericEvent(final MotionEvent event) {
                // Joysticks are supported since Honeycomb, but I don't care
                // about it, because very little devices have it
                if ((event.getSource() & InputDevice.SOURCE_CLASS_JOYSTICK) == InputDevice.SOURCE_CLASS_JOYSTICK) {
                    // event.getAxisValue(AXIS_HAT_X) and
                    // event.getAxisValue(AXIS_HAT_Y) are joystick arrow keys,
                    // they also send keyboard events
                    FFPlayerView.nativeGamepadAnalogJoystickInput(
                ...
                if (event.getAction() == MotionEvent.ACTION_SCROLL) {
                    ...
                    FFPlayerView.nativeMouseWheel(scrollX, scrollY);
                    return;
                }
    ...
    // grep 'SDLK_' SDL_keysym.h | sed 's/SDLK_\([a-zA-Z0-9_]\+\).*[=]
    // \([0-9]\+\).*/public static final int SDLK_\1 = \2;/' >> Keycodes.java
    public class SDL_1_2_Keycodes {
    ...
    // 's/SDL_SCANCODE_\([a-zA-Z0-9_]\+\).*[=] \([0-9]\+\).*/public static final
    // int SDLK_\1 = \2;/' >> Keycodes.java
